...
class EldersViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, UISearchControllerDelegate, UISearchResultsUpdating {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "eldersCell", for: indexPath) as! EldersTableViewCell
            ...
            if let rating = user["rating"] as? Double {
                if let email = user["email"] as? String {
                    ...
                    Database.database().reference().child("Users/\(safeEmail)/activities").observeSingleEvent(of: .value, with: { snapshot in
                        if let activities = snapshot.value as? [[String: Any]] {
                            cell.number.text = String(activities.count)
                        }
                        else {
                            cell.number.text = "0"
                        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "elderCommentsSegue", sender: request)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard let vc = segue.destination as? EldersCommentsViewController,
